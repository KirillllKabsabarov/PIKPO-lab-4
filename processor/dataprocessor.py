...
class DataProcessor(ABC):
    ...
    def run(self) -> None:
        ...
        cleaned_result = self.remove_col_by_name(self._dataset, ['Footnotes', 'Source'])
        ...
        self.result = self.get_dataframe_by_filter(cleaned_result, "Year >= 2019")
    ...
    def get_dataframe_by_filter(self, df: pandas.DataFrame, filter_expr: str) -> pandas.DataFrame:
        """
...
class CsvDataProcessor(DataProcessor):
    ...
    def __init__(self, datasource: str):
        ...
        DataProcessor.__init__(self, datasource)    # инициализируем конструктор родительского класса для получения общих атрибутов       # список допустимых разделителей
    ...
    def read(self):
        try:
            self._dataset = pandas.read_csv(self._datasource, sep=',', header='infer', names=None, encoding="utf-8", quoting=1)
            # Читаем имена колонок из файла данных
            col_names = self._dataset.columns
            # Если количество считанных колонок > 1 возвращаем True
            if len(col_names) > 1:
                print(f'Columns read: {col_names} using separator ,')
                return True
